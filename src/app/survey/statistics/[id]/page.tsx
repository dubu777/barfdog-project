...
export default function StatisticPage({params}: Props) {
  ...
  return (
    ...
    <section className={styles.statisticsMainTitleWrapper}>
      <h1 className={styles.statisticsMainTitle}>{formData.name}의 맞춤 레포트</h1>
    ...
    <section className={styles.confirmButtonWrapper}>
      <Link className={styles.confirmButton} href={`/order/subscribeShop?surveyReportsId=${id}`}>맞춤 플랜 확인하기</Link>
    </section>
    </div>
  );
}
